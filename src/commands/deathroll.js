...
module.exports = {
    ...
    async execute(discord_client, msg, args, admin) {
        ...
        if (args.length == 0) {
            let deathroll_guide = new EmbedBuilder()
            ...
            if (user?.deathroll_stats) {
                ...
                deathroll_guide.addFields({ name: 'Winrate', value: `Has won ${(wins / games * 100).toFixed(2)}% of ${games} games.`, inline: false });
                deathroll_guide.addFields({ name: 'Top Death', value: `You've died from ${top_death} odds.`, inline: false });
            }
            else {
                deathroll_guide.addFields({ name: 'No Games', value: 'Play games to see stats.', inline: false });
            }
        ...
        if (recipient == msg.author.id) {
            msg.channel.send(`${msg.author.username} - Can't challenge yourself... **+deathroll** for more info.`);
            return;
        }
        if (recipient == discord_client.user.id) {
            msg.channel.send(`${msg.author.username} - Can't challenge Beans... **+deathroll** for more info.`);
            return;
        }
        ...
        collector.on('end', async collected => {
            ...
            msg.channel.send(`Winner: ${user.user.username} - Payout: ${comma_adder.add(Math.trunc(cost))}`);
        });
