...
module.exports = {
    ...
    async execute(discord_client, msg, args, admin) {
        ...
        // else if (args.join(' ').toLowerCase().includes('counting')) {
        //     let cost = 1000000;
        //     if (!(await require('../utility/credits').transaction(discord_client, msg, cost))) return; // credits manager validates transaction

        //     let db_user = await require('../utility/queries').user(msg.guildId, msg.author.id);
        //     let counting_upgrade = db_user?.counting_upgrade ? +db_user.counting_upgrade : 1;

        //     counting_upgrade += 1;

        //     await db.doc(`servers/${msg.guildId}/members/${msg.author.id}`).set({
        //         counting_upgrade: counting_upgrade.toString(),
        //     }, { merge: true });

        //     let upgrade_embed = new EmbedBuilder()
        //         .setTitle(`Upgrade`)
        //         .setDescription(`Increased Counting Bonus`)
        //         .setColor('#000000')
        //         .addFields({ name: 'Counting Bonus Multiplier', value: `${counting_upgrade - 1}x -> **${counting_upgrade}x**`, inline: false })
        //         .addFields({ name: 'Cost', value: `1,000,000 credits`, inline: false })
        //         .setFooter({ text: `${msg.author.username}` })
        //         .setTimestamp();
    ...
    async upgrade_guide(msg) {
        ...
        let upgrade_guide = new EmbedBuilder()
            ...
            // .addFields({ name: '+upgrade counting', value: `Increases your counting bonus by 1\nCurrent: ${counting_upgrade}x`, inline: false })
            .setFooter({ text: `${msg.author.username}` })
            .setTimestamp();
