        ...
        try {
            switch (msg.channel.id) {
                case '1184186351260794880': // main-server // counting
                    ...
                    let date_bonus = (new Date()).getDate() == 1 || (new Date()).getDate() == 15 ? 10 : 1;
                    ...
                    let upgrade = db_user?.counting_upgrade ? db_user.counting_upgrade : 1;
                    if (!isNaN(number)) await require('../utility/credits').refund(discord_client, msg, msg.author.id, number * booster_bonus * date_bonus * upgrade); // credits manager refunds credits
