        ...
        try {
            switch (msg.channel.id) {
                case '1184186351260794880': // main-server // counting
                    ...
                    let date_bonus = (new Date()).getDate() == 29 || (new Date()).getDate() == 1 ? 2 : 1;
                    ...
                    if (!isNaN(number)) await require('../utility/credits').refund(discord_client, msg.author.id, number * booster_bonus * date_bonus); // credits manager refunds credits
                    break;
            }
        }
        catch (err) {
