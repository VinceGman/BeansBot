...
discord_client.on('messageCreate', async msg => {
  ...
  if (msg.guildId === null) {
    ...
    if (msg.channel.name != 'commands' && command != 'topic' && command != 'nsfw' && command != 'nsfwa' && command != 'koth' && msg.author.id != '183019001058689025') return; // owner : (await msg.guild.fetchOwner()).user.id
