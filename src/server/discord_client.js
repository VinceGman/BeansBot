...
discord_client.on('messageCreate', async msg => {
	...
	if (msg.guildId === null) {
		...
		try {
			...
			if (run_type == 'production' && (msg.content.toLowerCase().startsWith('dahlia') || msg.content.toLowerCase().startsWith('beans') || msg.mentions.users.has(discord_client.user.id) || reply)) {
